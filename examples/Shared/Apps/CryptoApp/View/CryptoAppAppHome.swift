...
struct CryptoAppAppHome: View {
    @State var currentCoin: String = "BTC"
    @Namespace var animation
    @StateObject var appModel: DCryptoAppAppViewModel = DCryptoAppAppViewModel()
            ...
            if let coins = appModel.coins,let coin = appModel.currentCoin{
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text("\(coin.price_change > 0 ? "+" : "")\(String(format: "%.2f", coin.price_change))")
                        ...
                        .background{
                            Capsule()
                                .fill(coin.price_change < 0 ? .red :Color("CoverLightGreen"))
                        }
                }
                .frame(maxWidth: .infinity,alignment: .leading)
    ...
    // MARK: Line Graph
    @ViewBuilder
    func GraphView(coin: CryptoAppAppCryptoModel)->some View{
        GeometryReader{_ in
            // See My Analytics App Video
            // Where I made About Custom Line Graph Video
            // Link in Description
            CryptoAppAppLineGraph(data: coin.last_7days_price.price,profit: coin.price_change > 0)
        }
        .padding(.vertical,30)
        .padding(.bottom,20)
    }
    ...
    @ViewBuilder
    func CustomControl(coins: [CryptoAppAppCryptoModel])->some View{
