...
struct ToolBarAnimationHome: View {
    // MARK: Sample Tools
    @State var tools: [ToolBarAnimationTool] = [
    ...
    @State var activeTool: ToolBarAnimationTool?
    ...
    @ViewBuilder
    func ToolView(tool: Binding<ToolBarAnimationTool>)->some View{
        HStack(spacing: 5){
            Image(systemName: tool.wrappedValue.icon)
                ...
                .background {
                    GeometryReader{proxy in
                        ...
                        Color.clear
                            .preference(key: ToolBarAnimationRectKey.self, value: frame)
                            .onPreferenceChange(ToolBarAnimationRectKey.self) { rect in
...
struct ToolBarAnimationHome_Previews: PreviewProvider {
...
struct ToolBarAnimationRectKey: PreferenceKey{
